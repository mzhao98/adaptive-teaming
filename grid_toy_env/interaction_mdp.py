...
action_to_text = {
    (SQUARE, G1): "square g1",
    (SQUARE, G2): "square g2",
    (TRIANGLE, G1): "triangle g1",
    (TRIANGLE, G2): "triangle g2",
    (ASK_PREF, SQUARE): "ask pref square",
    (ASK_PREF, TRIANGLE): "ask pref triangle",
    (ASK_DEMO, SQUARE): "ask demo square",
    (ASK_DEMO, TRIANGLE): "ask demo triangle",
}
...
class Interaction_MDP:
    ...
    def is_done_given_interaction_state(self, interaction_state):
        if (
            len(interaction_state["square_positions"]) == 0
            and len(interaction_state["triangle_positions"]) == 0
        ):
            return True
        else:
            return False
    ...
    def rollout_interaction(self):
        ...
        while not self.is_done_given_interaction_state(current_state):
            ...
            (
                next_state,
                seen_demos,
                possible_actions_to_prob_success,
                beliefs,
                reward,
                done,
            ) = self.step(
            ...
            (
                ...
                self.possible_actions_to_prob_success,
                self.seen_demos,
            ) = (beliefs, possible_actions_to_prob_success, seen_demos)
            if iters > 7:
                break
    ...
    def get_action(self, current_state):
        ...
        for action in self.possible_actions:
            ...
            ) = self.hypothetical_step_under_beliefs(
                current_state,
                action,
                init_beliefs,
                init_possible_actions_to_prob_success,
    ...
    def step_with_nn(self, input_state, action):
        ...
        if action in [
            ...
            target = {"target_object": action[0], "target_goal": action[1]}
            skill_state = {
                "pos": current_state["start_pos"],
                "holding": None,
                "objects_in_g1": None,
                "objects_in_g2": None,
            }
            state_vector = self.convert_state_to_vector(
                current_state, skill_state, target
            )
    ...
    def step(
        ...
        possible_actions_to_prob_success = copy.deepcopy(
            init_possible_actions_to_prob_success
        )
        ...
        step_cost = 5
        reward = (
            np.dot(feature_vector, self.true_human_reward_weights)
            - query_cost
            - step_cost
        )
    ...
    def hypothetical_step_under_beliefs(
        self,
        input_state,
        action,
        init_beliefs,
        ...
        reward = (reward_hyp1 * beliefs[tuple(hyps[0])]) + (
            reward_hyp2 * beliefs[tuple(hyps[1])]
        )
        # print("reward", reward)
        done = self.is_done_given_interaction_state(current_state)
        # if done:
    ...
    def randomly_initialize_bc_data(self, n_start=0):
        ...
        for instance in range(n_start):
            initial_config = {
                ...
                "g2": (4, 0),
                "square_positions": [
                    all_positions[np.random.choice(all_positions_indices)],
                    all_positions[np.random.choice(all_positions_indices)],
                ],
                "triangle_positions": [
                    all_positions[np.random.choice(all_positions_indices)],
...
if __name__ == "__main__":
    true_human_reward_weights = [
        1,
        -10,
        -10,
        1,
    ]  # [obj A(square) placed in G1, object A(square) in G2, B(triangle) in G1, B in G2]
