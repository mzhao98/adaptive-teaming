...
class NaiveGreedyPlanner(InteractionPlanner):
    ...
    def plan(
        ...
        for pref_idx in range(len(pref_space)):
            ...
            # if the robot has the skill, then the robot can complete the task
            # if the robot does not have the skill, then the robot will fail the task
            # the robot will fail the task with probability 1 - prob_skill
            prob_skill = 0
            for skill in self.robot_skills:
                if skill['task'] == task_seq[current_task_id] and skill['pref'] == pref_space[pref_idx]:
                    prob_skill = 1
                    break
            # candidate_reward is that given this is the correct preference, the robot will complete the task
            # with probability prob_skill
            # candidate_reward = -self.cost_cfg["ROBOT"] - self.cost_cfg["FAIL"] * (1 - prob_skill)
            # candidate reward is the likelihood of needing to ask for a skill
            # = probability of needing a skill * cost of asking for a skill
